    ...
    H5F_libver_t libver_mod, unsigned oh_vers_mod)
{
    ...
    H5O_native_info_t oinfo;            /* Object info */
    herr_t      ret;                    /* Return value */
     ...
     */
    ret = H5Oget_native_info_by_name(file, "/", &oinfo, H5O_INFO_HDR, H5P_DEFAULT);
    CHECK(ret, FAIL, "H5Oget_native_info_by_name");
    VERIFY(oinfo.hdr.version, oh_vers_create, "H5Oget_native_info_by_name");
    ...
    ret = H5Oget_native_info(group, &oinfo, H5O_INFO_HDR);
    CHECK(ret, FAIL, "H5Oget_native_info");
    VERIFY(oinfo.hdr.version, oh_vers_mod, "H5Oget_native_info");
...
test_libver_bounds_obj(hid_t fapl)
{
    ...
    ret = H5Oget_native_info_by_name(fid, "/", &oinfo, H5O_INFO_HDR, H5P_DEFAULT);
    CHECK(ret, FAIL, "H5Oget_native_info_by_name");
    ...
    for(low = H5F_LIBVER_EARLIEST; low < H5F_LIBVER_NBOUNDS; low++) {
        for(high = H5F_LIBVER_EARLIEST; high < H5F_LIBVER_NBOUNDS; high++) {
            ...
            if(fid >=0 ) { /* The file open succeeds */
                ...
                ret = H5Oget_native_info_by_name(gid, GRP_NAME, &oinfo, H5O_INFO_HDR, H5P_DEFAULT);
                CHECK(ret, FAIL, "H5Oget_native_info_by_name");
